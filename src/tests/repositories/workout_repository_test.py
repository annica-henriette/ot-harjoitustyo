...
class TestWorkoutRepository(unittest.TestCase):
    ...
    def test_list_all_user_workouts(self):
        ...
        workout_repository.create_workout(Workout(content="running", user=hupu.username))
        workout_repository.create_workout(Workout(content="gym", user=lupu.username))

        hupu_workouts = workout_repository.list_all_user_workouts(hupu.username)

        self.assertEqual(len(hupu_workouts), 1)
        self.assertEqual(hupu_workouts[0].content, "running")

        lupu_workouts = workout_repository.list_all_user_workouts(self.user_lupu.username)
